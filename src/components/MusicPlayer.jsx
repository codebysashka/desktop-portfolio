...
export const MusicPlayer = ({ isMaximized }) => {
	...
	return (
		<div className="flex h-full w-full select-none overflow-hidden text-white">
			...
			<div className={`flex h-full w-full ${isMaximized ? 'flex-row' : 'flex-col justify-start pt-2'}`}>
				<div className={`flex flex-col items-center shrink-0 ${isMaximized ? 'w-1/2 justify-center border-r border-white/10 p-6' : 'w-full px-4'}`}>
					...
					<div className={`relative shrink-0 flex items-center justify-center mb-2 ${isMaximized ? 'w-72 h-72' : 'w-40 h-40'}`}>
						...
						<div className={`rounded-full border-4 border-white/20 shadow-2xl overflow-hidden z-10 ${isPlaying ? 'animate-[spin_8s_linear_infinite]' : ''} ${isMaximized ? 'w-68 h-68' : 'w-36 h-36'}`}>
							<img src={currentTrack.cover} alt="" className="w-full h-full object-cover rounded-full" />
						</div>
					</div>

					<div className="text-center mb-2">
						<h3 className={`${isMaximized ? 'text-2xl' : 'text-lg'} font-bold`}>{currentTrack.title}</h3>
						<p className="text-sky-300 text-[10px] font-bold uppercase tracking-widest">{isPlaying ? 'Playing' : 'Paused'}</p>
					</div>
					...
					<div className="flex items-center gap-8 mb-4">
						...
						<button onClick={() => setCurrentTrackIndex((p) => (p + 1) % playlist.length)} className="text-white/40 hover:text-white transition-all">
							<svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z" /></svg>
						</button>
					</div>

					<div className="flex items-center gap-3 w-full max-w-50 mb-4 px-2 shrink-0">
